...
This was Ley's original app, built before `apps/ley`. It's archived here rather than kept as a
...
receive the fixes/dependency bumps `apps/ley` gets.

## Stack

- **App**: React Native + Expo (TypeScript), sharing `@ley/auth`/`@ley/ui` with `apps/ley`
...
- **Globe**: a `react-native-webview` hosting a bundled D3 (`d3-geo`) orthographic-projection canvas
  renderer (its own earlier version, predating the 3-tier Natural Earth pipeline `apps/ley` uses),
...
1. Create a Supabase project (or reuse an existing one — the schema is independent of `ley`'s).
