...
//
// A tile's fill reuses the ordinary closed-Polygon rendering, but drawn with the PLAIN (unsmoothed)
// path in globe-entry.js, never the corner-rounding smoothPath every other fill layer uses: a tile's
// own boundary is a mix of real coastline and the synthetic straight edges introduced by this exact
// slicing, and rounding a synthetic edge's corners (the smoothing's whole job) pulls that tile's
// fill inward from its true straight edge — invisible for one tile alone, but two adjacent tiles
// both pulling inward from the same shared edge opens a real gap between them, showing as a
// "sparkle"/star of ocean color where solid land should be (found live, confirmed by reproducing
// the exact combined-fill sequence outside the app — it reliably reappears the moment tile fill
// switches back to the smoothed path, and reliably vanishes with the plain one). Its OUTLINE can't
// reuse the tile's own boundary geometry either, for a different reason: stroking it would draw
// those same synthetic cut lines running through the middle of a continent as if they were real
// coastline. So each tile also gets its real-coastline-only edges split out separately (see
// realEdgeArcsOf below) — an edge whose endpoints both sit exactly on the tile's own clip boundary
// is synthetic and dropped; only edges that survive from the original ring are kept, as open
// LineString runs the renderer strokes (bbox-culled the same way as everything else, and smoothed
// like every other real coastline) instead of closing them into the tile shape itself.
